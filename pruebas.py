...
def get_sql_vouchers_tofix(state='posted'):
	return "select * from vouchers_tofix where voucher_id = 147778 and state='%s' and actionto = 'to_fix' order by voucher_id;" % state
...
try:
	...
	crsvouchers.execute(get_sql_vouchers_tofix())
	vouchers_tofix = crsvouchers.fetchmany(5)
	counter = 0
	...
	for vtofix in vouchers_tofix:
		...
		if voucher[0]['state'] == 'posted':
			models.execute_kw(db, uid, password, 'account.voucher', 'cancel_voucher', [int(voucherid)])

		#models.execute_kw(db, uid, password, 'account.voucher.line', 'unlink', [voucher[0]['line_cr_ids']])
		#
		# ai.id id_factura    	0
		# ai.move_id aml.id		1
		# move_line_id			2
		# ai.account_id			3
		# invoice_id			4
		# ai.internal_number	5
		# smr.id	smr_id		6
		# smr.voucher_id		7
		# smr.invoice_type		8
		# smr.num_mov			9
		# smr.amount_total		10
		# aml.amount_residual	11
		# ai.amount_residual	12
		# ai.amount_total		13
		# aml.name				14
		# aml.date				15
		# aml.date_maturity     16
		# credits				17
		# acc_receivable		18
		# invoice_number		19
		# invoice_number_serie	20
		...
		counter += 1
		print "Corrigiendo %s/%s VOUCHER ID %s, con Poliza %s y Referencias %s en la Sucursal %s" % \
					(counter, nfetched, voucherid, voucher[0]['number'], voucher[0]['name'], voucher[0]['voucher_operating_unit_id'])
...
#methodval = models.execute_kw( db, uid, password, 'account.voucher', 'cancel_voucher', [int(voucher_id)])
#methodval = models.execute_worflow( db, uid, password, 'account.voucher', 'action_cancel_draft', voucher[0]['id'] )

#print methodval
#print "Monto: %s. Facturas Pagadas: %s" % (voucher[0]['paid_amount_in_company_currency'],voucher[0]['line_cr_ids'])
