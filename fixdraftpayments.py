...
def get_vouchers():
	...
	sql = """
			...
			where av."type" = 'receipt' and avl."type" = 'dr' and avl.amount_unreconciled = 0
				and av.state = 'draft'
			order by avl.voucher_id		
		"""
...
def create_voucher(voucherid):
	...
	domain = [[['id', '=', voucherid]]]
	afieldsdpayment = ['id', 'partner_id', 'account_id', 'line_ids', 'voucher_operating_unit_id']
	fields = {'fields': afieldsdpayment}
	voucher = models.execute_kw(db, uid, password, 'account.voucher', 'search_read', domain, fields)
	print "Sucursal: %s, " % voucher[0]['voucher_operating_unit_id'][1],
...
def dopayments():
	...
	try:
		...
		for pago in pagos:
			ncounter += 1
			print "Corrigiendo %s/%s, Voucher ID: %s" % (ncounter, len(pagos), pago[0]),
